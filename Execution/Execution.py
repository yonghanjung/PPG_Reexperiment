...
class DrMPPGAnalysis:
    ...
    def Determine_PeakFindingBuffer(self, Int_PeakIdx, Int_PeakFindingBuffer, List_PeakCand):
        Array_PeakBuffer = np.array(range(Int_PeakIdx, Int_PeakIdx + Int_PeakFindingBuffer))
        PeakIsThere = False
        for PeakCand in List_PeakCand:
            if PeakCand in Array_PeakBuffer:
                PeakIsThere = True
                break
            else:
                pass
    ...
    def Execution(self):
        ...
        for IntIdx in range(Int_TotalSignalLength - Int_3secSignalLength):
            ...
            Int_BufferPeakFinding = Int_SSFBufferLength
    ...
    def ConvertFrequencyDomain(self):
        Array_Signal = self.Block_Signal()
